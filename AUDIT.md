...
### Post-audit reproduction baseline (starting HEAD `3f5ac86163a902d7c89e2adeebd332183196c0ed`)
...
### Post-audit hardening closure (2026-07-17)

This table closes the classifications above against the implementation commits created after starting HEAD `3f5ac86163a902d7c89e2adeebd332183196c0ed`. A checked box means the confirmed code-side work and its available automated coverage are complete. It does not convert any physical test into a pass.

| ID | Final disposition / status | Files and implementation commit | Automated evidence | Physical status |
|---|---|---|---|---|
| C01 | **ALREADY FIXED** — [x] Preserved the existing native self-test invocation; no duplicate path was added. | `dev-build-vcam.ps1`, `VirtualCamera_Installer/main.cpp`; verified throughout `95ef711`, `a6af0cb`, `99d2521` | Normal native build invokes all three self-test markers; packaging harness invokes it again explicitly. | **NOT YET TESTED** with external allocators |
| C02 | **CONFIRMED** — [x] Service-start failure now logs the original exception through `AppLogger`, publishes sanitized `lastError`, renders a Compose error, and exposes retry. | `MainActivity.kt`, `StreamViewModel.kt`; `13d0f84` | Android compile, 45/45 unit tests, debug/release APKs, and lint passed. | **NOT YET TESTED** with a deliberately failing physical service start |
| C03 | **CONFIRMED** — [x] MF decoder independently rejects access units over shared `OCB2_MAX_PAYLOAD`; exact-limit and over-limit tests do not depend on parser validation. | `mf_decoder.rs`, `ocb2.rs`; `13d0f84` | Producer 22/22 tests, including exact and oversized decoder inputs; release build passed. | Not a physical-only item |
| C04 | **CONFIRMED** — [x] Removed only the incorrect lock-held SAL annotation from `SimpleMediaStream::Stop`; runtime locking and `StopInternal` contract remain. | `SimpleMediaStream.cpp`; `13d0f84` | Release DLL compiled at `/W4` with zero warnings/errors. | **NOT YET TESTED** under external-consumer stop stress |
| C05 | **CONFIRMED** — [x] Reset is fail-fast by default, benign failures are scoped, elevation and step results are explicit, essential failures return nonzero, and completion text is impossible on failure. | `dev-reset.ps1`, `dev-tools/test-packaging.ps1`; `13d0f84`, `99d2521` | Non-destructive success/failure subprocess tests prove exit 0 vs 1 and absence of `Reset done.` on failure. | **NOT YET TESTED** as an elevated live reset |
| C06 | **PARTIALLY CONFIRMED** — [x] Completed producer/built/installed/registered/currently-loaded identity comparison, readiness gating, prominent mismatch diagnostics, and exact remediation. | `dev-build-vcam.ps1`, `virtualcam.rs`, `ControlPanel.tsx`; `95ef711`, `99d2521` | Native identity check and pure stale-combination Tauri tests passed. | **NOT YET TESTED** with an intentionally stale loaded DLL process |
| C07 | **PARTIALLY CONFIRMED** — [x] Added explicit production register/unregister/status/host/self-test/dev-menu commands; no args prints usage and Tauri reports elevation failures. | `InstallerCli.h`, installer `main.cpp`, `virtualcam.rs`; `95ef711`, `a6af0cb`, `99d2521` | Native CLI parser/exit tests plus seven packaging-policy subprocess checks passed. | **NOT YET TESTED** for real non-admin/admin registration transitions |
| C08 | **CONFIRMED** — [x] Production registration and host code now lives in `ProductionVirtualCamera.*`; the installer no longer compiles `VirtualCameraTest`. | `ProductionVirtualCamera.*`, `VirtualCamera_Installer.vcxproj`; `a6af0cb` | Production-only installer Release build passed with zero warnings/errors. | **NOT YET TESTED** for physical host activation |
| C09 | **CONFIRMED** — [x] Central poison recovery covers Tauri virtual-camera and preview state without changing audited lock order. | `sync_state.rs`, `virtualcam.rs`, `nv12_preview.rs`; `89d4b58` | Simulated panic/poison test proves later operations recover; Tauri 4/4 tests passed. | Not a physical-only item |
| C10 | **CONFIRMED** — [x] One machine-readable schema generates C++ and both Rust layouts, every field size/offset and total size is asserted, and fingerprint derives from canonical schema text. | `protocol/ring-abi.schema.json`, `generate-ring-abi.ps1`, generated C++/Rust files; `4ca257a` | Producer/Tauri unit tests and native compile execute the generated assertions in the normal matrix. | Not a physical-only item |
| C11 | **CONFIRMED** — [x] Kotlin, Rust, and the actual browser parser consume one nine-case OCB2 conformance corpus covering fragmentation, batching, invalid headers/lengths, reconnect, config, discontinuity/keyframe, EOS, and error. | `protocol/conformance/*`, `Ocb2.kt`, `Ocb2ConformanceTest.kt`, `ocb2.rs`, `ocb2-parser.js`; `9ecc84b` | Android corpus tests, producer tests, and `OCB2_BROWSER_CONFORMANCE=PASSED cases=9`. | **NOT YET TESTED** in a live WebCodecs browser stream |
| C12 | **CONFIRMED** — [x] Architecture state names and current Camera2/MediaCodec/OCB2/MF/NV12 design are documented. | `docs/ARCHITECTURE.md`; `a6af0cb` | Documentation reviewed against compiled production graph and current symbols. | Not a physical-only item |
| C13 | **CONFIRMED** — [x] Removed unread `StreamState.streaming` and every write. | `StreamState.kt`, `StreamService.kt`, `H264Streamer.kt`, `MjpegStreamer.kt`; `a6af0cb` | Repository search is empty; Android 45/45 tests/build/lint passed. | Not a physical-only item |
| C14 | **CONFIRMED** — [x] Deleted hard-coded live CORS probes and replaced them with deterministic selected-policy tests. | `ControlServerSecurityPolicyTest.kt`; `f4c19bf` | Seven auth/CORS boundary tests pass inside the Android suite. | **NOT YET TESTED** over a real LAN browser |
| C15 | **CONFIRMED** — [x] Rewrote stale README, architecture, validation, and native planning material for the V2 pipeline. | `README.md`, `docs/ARCHITECTURE.md`, `docs/VALIDATION.md`, native docs; `a6af0cb` | Documentation references current endpoints, formats, ring, fallback, CLI, and build graph. | Physical validation steps remain **NOT YET TESTED** |
| C16 | **CONFIRMED** — [x] No-argument inherited menu is gone; `--dev-menu` is explicit and reports that the sample menu is not in the production binary. | installer `main.cpp`, `InstallerCli.h`; `95ef711`, `a6af0cb` | Packaging subprocess proves no-args usage and explicit dev-menu behavior. | Not a physical-only item |
| C17 | **PARTIALLY CONFIRMED** — [x] After isolating production registration, HW/Augmented sources and activation branches were excluded from the production DLL. | `VirtualCameraMediaSource.vcxproj`, `VirtualCameraMediaSourceActivate.*`; `a6af0cb` | Release DLL production graph builds with zero warnings/errors. | **NOT YET TESTED** through external MF enumeration |
| C18 | **PARTIALLY CONFIRMED** — [x] Created the dependency map before cleanup; production solution contains only DLL/installer and unbuilt Microsoft projects live under `upstream-samples` with attribution. | `docs/NATIVE_DEPENDENCY_MAP.md`, `VirtualCameraSample.sln`, `upstream-samples/*`; `a6af0cb` | Both production projects build; repository references and project inclusions were checked before moves. | Not a physical-only item |
| C19 | **CONFIRMED** — [x] Added HTTP/auth boundary tests for LAN token, `/health`, loopback mutations, bind snapshot, allowlist CORS, and selected constant-time comparison. | `ControlServerSecurityPolicy.kt` and test; `f4c19bf` | Seven policy tests pass in Android 45/45 suite. | **NOT YET TESTED** from a second LAN device |
| C20 | **PARTIALLY CONFIRMED** — [x] Added lifecycle, priority, bounded backlog, coalescing, cancellation/completion, stale generation, reconfigure, failure, and recovery tests. | `PipelineController.kt`, `StreamService.kt`, `PipelineControllerTest.kt`; `ea6e964` | Eight state-machine tests pass in Android 45/45 suite. | **NOT YET TESTED** under repeated physical reconnect/reconfigure stress |
| C21 | **CONFIRMED** — [x] Extracted the desktop synchronization policy and applied it to actual UI flow. | `pipelineSyncPolicy.*`, `ControlPanel.tsx`; `443606e` | Six frontend tests cover 409 rollback, 422 alternatives, SSE/poll convergence, selected/actual launch, missing tuple, and cross-client identity. | **NOT YET TESTED** against simultaneous live clients |
| C22 | **PARTIALLY CONFIRMED** — [x] Added a local mock-phone HTTP integration test for canonical fallback, authenticated request bodies, 409 retry, 422 alternative, and accepted tuple propagation. | producer `main.rs`; `931534f` | Producer 22/22 tests; integration scenario ends H.264 1080p60 at accepted MJPEG 1280x720@30 after exercising 1080p30 and alternative negotiation. | **NOT YET TESTED** with a live decoder-triggered phone fallback |
| C23 | **PARTIALLY CONFIRMED** — [x] Added complete CLI/exit policy, stale identity, host-readiness, native self-test, and reset exit coverage without mutating machine registration during unit tests. | `InstallerCli.h`, `virtualcam.rs`, `dev-tools/test-packaging.ps1`; `99d2521` | Tauri 4/4, native three-part self-test, and seven packaging subprocess checks passed. | Real admin/non-admin registration and host activation remain **NOT YET TESTED** |
| C24 | **PARTIALLY CONFIRMED** — [x] Corrected UUIDv4 entropy documentation to 122 random bits and retained the secure generator; selected token comparison is regression-tested. | `SettingsManager.kt`, auth policy test; `a6af0cb`, `f4c19bf` | Android auth tests pass. | Not a physical-only item |
| C25 | **FALSE POSITIVE** as a blocker — [x] No correctness defect or measured resource failure was reproduced for the stated 1–3 SSE clients; observation retained without speculative redesign. | Classification/evidence only; `8e6717c` | Static reproduction documented; no code mutation justified. | Sustained multi-client load is **NOT YET TESTED** |
| C26 | **FALSE POSITIVE** as a blocker — [x] No defect was reproduced in high-speed zoom burst rebuilding at current cadence; behavior intentionally preserved. | Classification/evidence only; `8e6717c` | Existing controller/capture policy tests remain green. | Physical smooth-zoom/high-speed stress is **NOT YET TESTED** |
| C27 | **ALREADY FIXED** implementation / **CONFIRMED** coverage gap — [x] Preserved the selected auth design and added its missing regression suite. | `ControlServerSecurityPolicy.*`; `f4c19bf` | Seven auth/CORS tests pass in Android 45/45 suite. | **NOT YET TESTED** over live LAN |

No post-audit finding is intentionally deferred on the code side. Items C25 and C26 were not changed because reproduction found no code defect; their stated stress/scalability concerns remain explicit physical/performance checks rather than fabricated failures.
...
- 2026-07-17 post-audit hardening — Android `testDebugUnitTest assembleDebug assembleRelease lint`: **PASSED, 45/45**, with zero test failures/errors/skips; debug and unsigned release APKs completed and full lint passed after declaring the Camera2 interop contract explicitly. Kotlin compilation fell back in-process because the sandbox could not create its daemon marker outside the workspace; that diagnostic did not fail compilation or any task.
- 2026-07-17 post-audit hardening — producer `cargo fmt --check`, `cargo test --all-targets`, and `cargo build --release`: **PASSED, 22/22**. New HTTP integration evidence covers authenticated canonical fallback request bodies, 409 retry, 422 alternatives, and accepted tuple propagation in addition to OCB2/ABI/decoder/ring/pacing coverage.
- 2026-07-17 post-audit hardening — Tauri backend `cargo fmt --check`, `cargo test --all-targets`, and `cargo build --release`: **PASSED, 4/4**, including poison recovery, complete host/registration readiness, ring ABI, and stale binary combinations.
- 2026-07-17 post-audit hardening — frontend `npm run typecheck`, `npm test`, and `npm run build`: **PASSED**; six synchronization tests, nine shared browser OCB2 corpus cases, TypeScript check, and Vite production build completed.
- 2026-07-17 post-audit hardening — native `dev-build-vcam.ps1 -NoKill` at source `95fe349c24b00ff3b3f8f45ba32f4f53188d2d61`: **PASSED**, zero warnings/errors. ABI fingerprint `0x7875b6e5eb257f55`; buffer-lock, NV12-resize, and installer-CLI self-tests passed. Built, installed, and registry-selected DLL SHA-256 values matched `17B5F0033C673C7D5F77F5F04BB496BD7B6DD0A177A4E0CCF20F82B2EC6AB6C0`; producer SHA-256 was `57EB5236E455249E1103F2DFA5E0E433A8C48AC4C3E9D21F5376C9F23A35E36F`. Current-ring loaded identity was **SKIPPED / NOT READY**, not passed, because the restricted verification identity could not open `C:\ProgramData\OpenCamBridge\framebuffer.bin` and no current heartbeat was established.
- 2026-07-17 post-audit hardening — `dev-tools/test-packaging.ps1`: **PASSED, 7/7** for no-argument usage, invalid exit 2, read-only status, explicit dev-menu, three native self-tests, reset success exit 0, and essential-failure exit 1 with no false completion message. Real register/unregister mutation was intentionally excluded from ordinary non-admin automation.
...
- 2026-07-17 post-audit hardening — OnePlus 9 regular Camera2, constrained high-speed direct surface, constrained high-speed GPU bridge, live 1080p60/720p60, phone preview transforms, MJPEG compatibility, thermal behavior, 30-minute screen-off operation, USB removal/reconnect, and simultaneous phone/web/Tauri control remain **NOT YET TESTED**.
- 2026-07-17 post-audit hardening — OBS and FFmpeg visible pixels, all advertised NV12/RGB32 modes, external allocator lock fallthrough, abnormal consumer exit, virtual-camera host activation, real admin/non-admin register/unregister, and current-process loaded-DLL identity remain **NOT YET TESTED**. Automated readiness and policy tests are not substituted for those physical checks.
