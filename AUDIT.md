...
| 4 | [x] One buffer-lock abstraction tries `IMF2DBuffer2::Lock2DSize`, `IMF2DBuffer::Lock2D`, then `IMFMediaBuffer::Lock`; it retains scanline/buffer start/length/pitch/unlock state and validates NV12/RGB32 destination bounds. | `SharedMemoryClient.cpp`, `SimpleMediaStream.cpp`; `dac442c`, `caf9bd3` | Native DLL/host rebuild passed with zero warnings/errors; producer ring bounds/invalid-metadata tests passed | **NOT YET TESTED** with an external physical camera consumer |
...
| 12 | [x] Producer readiness has explicit connection/config/keyframe/decode/ring states. Start rejects unless the producer is running, is `WRITING_RING`, has at least three commits, and passes readiness. Virtual-camera readiness requires recent consumer heartbeat, sample requests, successful ring reads, and accepted-sequence progress with PID/generation baseline resets. | `main.rs`, `virtualcam.rs`, `ControlPanel.tsx`; `dac442c`, `e69dcd2` | Producer **16/16**, including stale-readiness/PID-reset tests; Tauri **1/1**; Rust/frontend release builds passed | **NOT YET TESTED** for live connect/consume/reconnect or abnormal OBS exit |
...
| # | Code status | Files / implementation commit | Automated evidence | Live-test status |
|---|---|---|---|---|
| 13 | [x] H.264 rotation/mirror authority is carried in OCB2 stream info as `effectiveRotation`, `mirror`, `sensorOrientation`, and `deviceRotation`. Android computes it from sensor/device/manual state; a transform change performs a generation/discontinuity restart; both Windows decode paths apply the OCB2 values directly to NV12. MJPEG retains its phone-side transform. | `FrameTransform.kt`, `H264Streamer.kt`, `ocb2.rs`, `main.rs`, `ControlPanel.tsx`; `d2776fd`, `e69dcd2` | Android transform tests **3/3**; producer NV12 rotate/mirror and OCB2 transform-validation tests passed | **NOT YET TESTED** for visible 90/180/270 and front/back mirror output on a phone/OBS |
| 14 | [x] Producer launch failures propagate. Start rejects when the child is absent, state is not `WRITING_RING`, fewer than three frames committed, readiness is false, or the deadline expires. Optimistic success/error clearing was removed, so the primary action remains retryable after failure. | `ControlPanel.tsx`, `virtualcam.rs`; `e69dcd2` | TypeScript/Vite production build and Tauri Rust test/release build passed | **NOT YET TESTED** with an intentionally broken live producer executable/stream |
...
| 22 | [x] Media sample writing uses an RAII lock abstraction with the required `IMF2DBuffer2::Lock2DSize` → `IMF2DBuffer::Lock2D` → `IMFMediaBuffer::Lock` chain and validated NV12/RGB32 offsets, lengths, pitch, and unlock behavior. | `SimpleMediaStream.cpp`, `SharedMemoryClient.cpp`; `caf9bd3` | Media Foundation DLL/host release rebuild passed with zero warnings/errors; installed/loaded hash matched | **NOT YET TESTED** across multiple external consumers/sample allocator implementations |
...
| 24 | [x] `PipelineController` uses a bounded/coalescing pending queue rather than one suspended sender coroutine per action. Apply-settings and preview/control changes collapse to the newest complete state, stop/recover/failure receive priority, and superseded/closed commands complete explicitly as `CANCELLED`. | `PipelineController.kt`, `StreamService.kt`, `ControlServer.kt`; `d2776fd` | Android **9/9**, compile and APK build passed | **NOT YET TESTED** with rapid physical slider/reconfigure/recover stress |
